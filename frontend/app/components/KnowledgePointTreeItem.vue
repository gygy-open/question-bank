...
import type { KnowledgePointNode } from '@/lib/knowledgePointTree'
...
const toggleSelect = () => emit('toggle', String(props.node.id))
...
<template>
  <div>
    <div
      ...
      @click="toggleSelect"
    >
      <div class="mr-1 p-0.5 rounded-sm hover:bg-muted/80 text-muted-foreground/70 shrink-0" @click.stop="toggleOpen">
      ...
      <FolderOpen v-if="isExpanded && hasChildren" class="w-4 h-4 mr-2 text-primary shrink-0" />
