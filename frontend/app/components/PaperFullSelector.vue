...
import { Search, Check, Plus, Loader2 } from 'lucide-vue-next'
...
const confirmAdd = async () => {
  ...
  try {
    ...
    await addItems(selectedPaperId.value, props.questionIds)
...
const createNewPaper = async () => {
  ...
  try {
    ...
    await addItems(paper.id, props.questionIds)
          ...
          >
            ...
            <div class="flex-1 min-w-0">
              ...
              <div class="text-sm text-muted-foreground mt-1">
                {{ paper.question_count }} 题 · {{ formatRelativeTime(paper.updated_at) }}
              </div>
            </div>
          </button>
