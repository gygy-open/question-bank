...
5. **迁移实现**:MD→RichDoc 与答案转换统一用 **Python 转换器**(`app/services/question_content_converter.py`),Alembic data-migration 直接调用它,不再依赖前端一次性 Node 脚本。
...
4. **Revision 2(快照 + 数据转换,data-only,内联 `sa.table`)**— revision `a7b8c9d0e1f2`,down_revision `f1a2b3c4d5e6`,全程幂等、可重跑:
  ...
  - c. 逐行调 `app/services/question_content_converter.py` 转换六个内容字段 + options 写回 `questions`(内容字段存 JSON **字符串**、`options` 存 JSON **对象**;全程 Core,JSON 参数在 SQLite/MySQL 一致),成功即置 `content_schema_version = 1`。
